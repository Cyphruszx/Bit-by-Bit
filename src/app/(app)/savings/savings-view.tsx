  ...
  return (
    <>
      ...
      <article className="mt-8 card p-6">
        ...
        <div className="mt-5">
          {pots.length === 0 ? (
            <p className="text-sm text-muted">Add a pot to see the path to target.</p>
          ) : (
      ...
      <section className="mt-8 grid gap-4 lg:grid-cols-3">
        {pots.length === 0 ? (
          <p className="text-sm text-muted">No savings pots yet. Add one above.</p>
        ) : (
          pots.map((pot) => (
            <PotCard
              key={pot.id}
...
function PotCard({
  ...
  return (
    <article className={`card p-6 ${included ? "" : "opacity-70"}`}>
      {editing ? (
        <form
          onSubmit={(event) => {
            event.preventDefault();
        ...
        <>
          <div className="flex items-start justify-between gap-3">
            ...
            {included ? null : (
              <span className="rounded-full bg-surface-subtle px-2 py-0.5 text-xs font-medium text-muted">Hidden</span>
            )}
          </div>
          <p className="mt-1 text-sm text-muted">{pot.detail}</p>
          <p className="mt-5 text-2xl font-bold">{formatAud(pot.saved)}</p>
          <p className="mt-1 text-sm text-muted">of {formatAud(pot.target)}</p>
  ...
  return (
    <label className="block min-w-0 text-sm">
      ...
      <input
        ...
        className="w-full rounded-full border border-line px-3 py-2 text-sm outline-none focus:border-primary"
      />
    </label>
  );
}
