...
describe("AI JSON helpers", () => {
  ...
  it("turns a receipt extract into one signed expense", () => {
    ...
    assert.equal(result.transactions[0].categoryKey, "food.groceries");
  ...
  it("snaps a loose label onto a real category, and refuses to invent one", () => {
    assert.equal(snapCategory("groceries"), "food.groceries");
    assert.equal(snapCategory("food.groceries"), "food.groceries");
    assert.equal(snapCategory("Food & Drink"), "food");
...
describe("initial AI tagging", () => {
  it("only retags movements nothing else could settle", () => {
    const { transactions, taggedCount } = applyTagSuggestions(
      [txn(), txn({ id: "2", merchant: "Woolworths", categoryKey: "food.groceries", tags: ["Groceries"] })],
      [
    ...
    assert.equal(transactions[1].categoryKey, "food.groceries");
...
describe("image interpretation with AI", () => {
  it("uses AI vision and skips OCR when the model finds activity", async () => {
    ...
    assert.equal(parsed.transactions[0].categoryKey, "food.restaurants");
