...
describe("the order the app is allowed to decide", () => {
  ...
  it("counts a movement with no category as one still to be looked at", () => {
    ...
    assert.ok(!needsReview(txn({ categoryKey: "food.restaurants" })));
...
describe("learning from a correction", () => {
  it("remembers a merchant, whatever case the statement wrote it in", () => {
    const rules = learn({}, txn(), "food.restaurants", "2026-07-01T00:00:00.000Z");
    assert.equal(ruleFor(rules, txn({ merchant: "KFC" }))?.categoryKey, "food.restaurants");
    assert.equal(ruleFor(rules, txn({ merchant: "  kfc " }))?.categoryKey, "food.restaurants");
  ...
  it("keeps what it changed, so the list can say what it learned", () => {
    const rules = learn({}, txn({ categoryKey: "shopping" }), "food.restaurants", "2026-07-01T00:00:00.000Z");
  ...
  it("reads back as sentences, commonest first, with what each one is holding", () => {
    const rules: Rules = {
      ...learn({}, txn(), "food.restaurants", "2026-07-01T00:00:00.000Z"),
      ...learn({}, txn({ merchant: "Woolworths" }), "food.groceries", "2026-07-02T00:00:00.000Z"),
    };
    ...
    assert.deepEqual(
      ...
      [
        ["Woolworths is Food & Drink · Groceries", 2],
        ["Kfc is Food & Drink · Restaurants", 1],
      ],
    );
  });
...
describe("walking the ladder", () => {
  it("applies a correction to every movement of that merchant", () => {
    ...
    for (const row of rows) {
      assert.equal(row.categoryKey, "food.restaurants");
  ...
  it("never re-decides a movement the person settled themselves", () => {
    const rules = learn({}, txn(), "food.restaurants", "2026-07-01T00:00:00.000Z");
    const chosen = txn({ categoryKey: "leisure.events", decidedBy: "said" });
    assert.deepEqual(classify([chosen], { rules }), [chosen]);
  });

  it("beats the rules table, because a person outranks a regex", () => {
    const rules = learn({}, txn(), "leisure.events", "2026-07-01T00:00:00.000Z");
    const guessed = txn({ categoryKey: "food.restaurants", decidedBy: "rules" });
    assert.equal(classify([guessed], { rules })[0].categoryKey, "leisure.events");
  });
  ...
  it("carries a correction across from a movement the person settled, with no rule stored", () => {
    ...
    const rows = classify([
      txn({ id: "chosen", categoryKey: "food.restaurants", decidedBy: "said" }),
      txn({ id: "other" }),
    ]);
    assert.equal(rows[1].categoryKey, "food.restaurants");
    assert.equal(rows[1].decidedBy, "merchant");
  });

  it("re-derives the type from the category and the direction", () => {
    const rules = learn({}, txn({ merchant: "Ato" }), "income.rebate", "2026-07-01T00:00:00.000Z");
