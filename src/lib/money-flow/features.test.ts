...
describe("Spec 12 OPEN_BANKING feature gate", () => {
  it("is a Spec 5 key, MVP default on, and not on the Core enable offer", () => {
    assert.equal(isFeatureKey("OPEN_BANKING"), true);
    assert.equal(FEATURE_DEFAULTS.OPEN_BANKING, true);
    assert.equal(isFeatureEnabled(undefined, "OPEN_BANKING"), true);
    assert.equal(hasOpenBankingBundle(undefined), true);
  ...
  it("stays on for empty or missing keys and honours an explicit turn-off", () => {
    assert.equal(hasOpenBankingBundle({ OPEN_BANKING: true }), true);
    assert.equal(hasOpenBankingBundle({ OPEN_BANKING: false }), false);
    assert.equal(hasOpenBankingBundle({ POOLS: true }), true);
    assert.equal(hasOpenBankingBundle({}), true);
...
describe("Spec 12 OPEN_BANKING panel UX", () => {
  ...
  it("shows Connect bank by default and hides it after an explicit turn-off", () => {
    assert.equal(canShowConnectBank(FEATURE_DEFAULTS.OPEN_BANKING), true);
    assert.equal(canShowConnectBank(isFeatureEnabled(undefined, "OPEN_BANKING")), true);

    const disabled = recordFeatureToggle(EMPTY_LEDGER, "OPEN_BANKING", false);
    assert.equal(disabled.featureToggles?.OPEN_BANKING, false);
    assert.equal(isFeatureEnabled(disabled.featureToggles, "OPEN_BANKING"), false);
    assert.equal(canShowConnectBank(isFeatureEnabled(disabled.featureToggles, "OPEN_BANKING")), false);

    const enabled = recordFeatureToggle(disabled, "OPEN_BANKING", true);
