...
**Status:** reference only. Do not implement from this file until the user confirms which numbered steps to run.
...
Client-only Next.js app. Uploads, tags, charts, period filter, recurring, and savings persist in `localStorage` (`bitbybit.interpreted-v1` and related keys).
...
| Auth | Not wired in the running app |
| Supabase | Migration + RLS drafted in `supabase/migrations/202608210001_initial_finance_schema.sql`; unused at runtime |
| AI key | Server-only `OPENAI_API_KEY` (must never be `NEXT_PUBLIC_`) |
| Blast radius today | XSS or a shared browser can dump every interpreted statement, recurring item, and savings pot |
...
- In: `[feature / user outcome]`
- Out: `[explicit non-goals]`
- Success: `[what you can see or measure]`

### 2. Auth

- Sign-in: `[email | magic link | other]`
- Session: `[httpOnly, Secure, SameSite=Lax cookies | Supabase SSR cookies]`
- Demo after login: `[keep | drop | optional]`

### 3. Persistence

- Move off `localStorage`: `[transactions | recurring | savings | period | all]`
- Existing Supabase tables: `[yes as-is | migrate first]`
- Schema gaps: `[primary/sub-tags | recurring | savings | none]`
...
- Start with: `[number from above]`
- Stop after: `[number]`

Suggested first slice (not approved): **§11–13** (Privacy + Terms + consent), then **§7** (vision vendor, keep Tesseract, never send structured files to AI). Any persist/auth work starts at **§2–3** and must pass **§10.11** before production.

---

## Open decisions

Leave these blank until the user fills them. Do not guess in a PR.

- Auth method and session storage
- Whether to persist to Supabase next, and which stores move first
- AI vendor (Gemini Flash vs keep OpenAI vs Azure receipts)
- Application-layer encryption now vs documented residual risk
